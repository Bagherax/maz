...
const RegisterForm: React.FC<RegisterFormProps> = ({ onSocialLogin }) => {
  ...
  const validateStep1 = () => {
    ...
    if (!data.name || data.name.length < 3) newErrors.name = t('auth.error_username_short'); // Check name
  ...
  const Step1_BasicInfo = () => (
    <div className="space-y-4">
        ...
        <SocialAuthButtons onSocialLogin={onSocialLogin} />
    </div>
  );
